...
解决方式：
- 方案一：查询数据库 -》 放入缓存操作 加锁。
- 方案二：热点数据不过期。（物理不过期，业务上可以加个过期时间字段）
	![[Redis相关问题-1752820499859.png|536x292]]


# 缓存雪崩

定义：某个时刻，大量 `key` 过期，大量请求打到数据库中导致系统崩溃。
...
内存使用策略：
- `redis.conf` 中的 `maxmemory` 设置内存大小（默认无限制）
- 当内存达到 `maxmemory` 所设置的值时，可以使用如下的淘汰策略 `maxmemory-policy` （默认是 `noeviction` 不淘汰）：
	![[Redis相关问题-1752821282579.png]]

# Redis 中的 String 最大能放多大？

- `String` 最大能放 512MB；
- `List` 类型，最大能放 2^32 -1
- `Set` 类型，最大能放 2^32 -1
- `Hash` 类型，最大能放 2^32 -1 个键值对
